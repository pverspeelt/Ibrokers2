...
tws_connect <-
  function (clientId = 1L, host = "localhost", port = 7496, verbose = TRUE,
    ...
    if (is.null(filename)) {
      ...
      #   msg <- paste0('v', .client_version$MIN_CLIENT_VER, "..", .client_version$MAX_CLIENT_VER)
      # }
      ...
      while (TRUE) {
        ...
        if (is.null(SERVER_VERSION)) {
          ...
          CONNECTION_TIME <- readBin(sock_con, character(), 1)
          next
        }
      }
      ...
      tws_con$connected <- NULL # check python / c++ code if this is used
      ...
      return(tws_con)
        ...
        tws_con$port <- NULL
        tws_con$server.version <- NULL
        tws_con$connected.at <- filename
